package com.neuroandroid.pyweather.widget.tablayout;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private float mTextSideSize;
    private int mTextSideColor;
    /*********************************/
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextSideSize = ta.getDimension(R.styleable.SlidingTabLayout_tl_textSideSize, sp2px(14));
        /***********************************/
        ...
        mTextSideColor = ta.getColor(R.styleable.SlidingTabLayout_tl_textSideColor, mTextUnselectColor);
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            if (tv_tab_title != null) {
                ...
                } else if (i == mCurrentTab + 1 || i == mCurrentTab - 1) {
                    tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextSideSize);
                    tv_tab_title.setTextColor(mTextSideColor);
                } else {
                    tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize);
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        // Log.i("main", "positionOffset : " + positionOffset);
        scrollToCurrentTab();
        // updateTabSelection();
        invalidate();
    }
    ...
    private void updateTabSelection(int position) {
        // Log.i("main", "updateTabSelection : " + position);
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                ...
                if (i == position) {
                    ...
                    if (tab_title.getTextSize() != mTextSelectSize) {
                        valueAnimator = ValueAnimator.ofFloat(mTextSideSize, mTextSelectSize);
                        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                            @Override
                            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                                tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, (Float) valueAnimator.getAnimatedValue());
                            }
                        });
                        valueAnimator.setDuration(200);
                        valueAnimator.start();
                    }
                    tab_title.setTextColor(mTextSelectColor);
                } else if (i == position + 1 || i == position - 1) {
                    // tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextSideSize);
                    if (tab_title.getTextSize() != mTextSideSize) {
                        valueAnimator = ValueAnimator.ofFloat(tab_title.getTextSize(), mTextSideSize);
                        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                            @Override
                            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                                tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, (Float) valueAnimator.getAnimatedValue());
                    ...
                    tab_title.setTextColor(mTextSideColor);
                } else {
    ...
    private void updateTabSelection() {
        /**
         * 向右滑动：position大部分与目标页面position一致  翻页不成功->position变为原页面position
         * 翻到上一页 positionOffset [1->0]
         *
         * 向左滑动：position大部分与当前页面position一致  翻页不成功->position变为原页面position
         * 翻到下一页 positionOffset [0->1->0]
         */
        for (int i = 0; i < mTabCount; ++i) {
            View tabView = mTabsContainer.getChildAt(i);
            ...
            if (tab_title != null) {
                ...
                if (mTextBold == TEXT_BOLD_WHEN_SELECT) {
                    tab_title.getPaint().setFakeBoldText(i == mCurrentTab);
                }
            }
        }
    }
    ...
    public void setSelectTextsize(float selectTextsize) {
        this.mTextSelectSize = sp2px(selectTextsize);
        updateTabStyles();
    }
