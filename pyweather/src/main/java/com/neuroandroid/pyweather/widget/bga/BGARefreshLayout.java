...
package com.neuroandroid.pyweather.widget.bga;
...
public class BGARefreshLayout extends LinearLayout {
        ...
        if (refreshDiffY > 0 && shouldHandleRefresh() && isCustomHeaderViewCompleteVisible()) {
            ...
            if (paddingTop > 0 && mCurrentRefreshStatus != RefreshStatus.RELEASE_REFRESH) {
                ...
                if (mCurrentRefreshStatus != RefreshStatus.PULL_DOWN) {
                    if (mStatusRefreshLayoutDelegate != null) {
                        mStatusRefreshLayoutDelegate.onBeforeRefreshing();
                    }
    ...
    private void hiddenRefreshHeaderView() {
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mStatusRefreshLayoutDelegate != null) {
                    mStatusRefreshLayoutDelegate.onAfterOrReleaseRefreshing();
                }
            }
        });
        animator.start();
    ...
    private BGAStatusRefreshLayoutDelegate mStatusRefreshLayoutDelegate;

    public void setStatusRefreshLayoutDelegate(BGAStatusRefreshLayoutDelegate statusRefreshLayoutDelegate) {
        mStatusRefreshLayoutDelegate = statusRefreshLayoutDelegate;
    }

    public interface BGAStatusRefreshLayoutDelegate {
        /**
         * 下拉刷新之前需要处理的事情
         * 比如隐藏TitleBar
         */
        void onBeforeRefreshing();

        /**
         * 当释放下拉刷新或者下拉刷新结束的时候需要处理的事情
         * 比如刷新结束或者释放时可以显示TitleBar
         */
        void onAfterOrReleaseRefreshing();
    }
