 ...
 * 通用标题栏
 * 分为 左、中、右三部分
 * Created by LML on 2016-07-17.
 */
public class TitleBar extends ViewGroup implements View.OnClickListener {
    private static final int DEFAULT_MAIN_TEXT_SIZE = 18;   // 默认中间字体大小
    private static final int DEFAULT_SUB_TEXT_SIZE = 12;    // 默认中间子标题大小
    private static final int DEFAULT_ACTION_TEXT_SIZE = 15; // 默认右侧字体大小
    private static final int DEFAULT_TITLE_BAR_HEIGHT = 48; // 默认标题栏高度
    ...
    private void init(Context context) {
        ...
        mHeight = (int) UIUtils.getDimen(R.dimen.y88);
        initView(context);
    }
    ...
    private void initView(Context context) {
        ...
        mCenterText.setIncludeFontPadding(false); // 上下标
    ...
    public void setTextColor(int resId) {
        ...
        mLeftText.setTextColor(resId);
        mActionTextColor = resId;
    }
    ...
    private void setLeftClickListener(OnClickListener l) {
    ...
    public void setImageTitle(int resId) {
        ...
        mCenterText.getLayoutParams().height = (int) getResources().getDimension(R.dimen.y32);
    ...
    private static final int LEFT_ACTION = 33;
    private static final int RIGHT_ACTION = 34;
    ...
    public View addRightAction(Action action, int index) {
        ...
        View view = inflateAction(action, RIGHT_ACTION);
    ...
    public View addLeftAction(Action action, int index) {
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT,
                LayoutParams.MATCH_PARENT);
        View view = inflateAction(action, LEFT_ACTION);
     ...
     * @return a view
     */
    private View inflateAction(Action action, int dir) {
        View view = null;
        if (TextUtils.isEmpty(action.getText())) {
            if (dir == LEFT_ACTION) {
                view = View.inflate(getContext(), R.layout.item_message_action, null);
                ImageView img = (ImageView) view.findViewById(R.id.iv);
                img.setImageResource(action.getDrawable());
            } else {
                view = View.inflate(getContext(), R.layout.item_point_action, null);
                ImageView img = (ImageView) view.findViewById(R.id.iv);
                img.setImageResource(action.getDrawable());
            }
        } else {
            TextView text = new TextView(getContext());
            text.setBackgroundResource(R.drawable.return_ripple);
            ...
            text.setOnLongClickListener(view1 -> {
                if (!UIUtils.isEmpty(text.getText().toString())) {
                    ShowUtils.showToast(text.getText().toString());
                }
                return true;
            });
            view = text;
     ...
     * @param show  是否显示红点
     */
    public void showOrHideRedPoint(Action action, boolean show) {
        View view = getViewByAction(action);
        View redPoint = view.findViewById(R.id.view_point_red);
        redPoint.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
    }
     ...
     * @param show  是否显示数字圆点
     */
    public void showOrHideNumPoint(Action action, boolean show) {
        View view = getViewByAction(action);
        // TextView tv = (TextView) view.findViewById(R.id.tv_red_point);
        View redPoint = view.findViewById(R.id.view_point_red);
        redPoint.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
    }
     ...
     * @param num  设置数字圆点文本
     */
    private void setNumPointText(Action action, String num) {
        /*View view = getViewByAction(action);
        TextView tv = (TextView) view.findViewById(R.id.tv_red_point);
        tv.setText(num);*/
    }
